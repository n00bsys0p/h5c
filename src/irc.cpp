...
void ThreadIRCSeed()
{
    ...
    RenameThread("HighFiveCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #HighFiveCoinTEST0\r");
            Send(hSocket, "WHO #HighFiveCoinTEST0\r");
        } else {
            // randomly join #HighFiveCoin00-#HighFiveCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #HighFiveCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #HighFiveCoin%02d\r", channel_number).c_str());
        }
